...
# Проверка работоспособности заглушки на проде БЕЗДУМНО НЕ ИСПОЛЬОВАТЬ
prod-check-maintenance: ## Имитация работ на проде для проверки заглушки
	@echo "🛠️ Имитируем технические работы на Jino..."
	@ssh $(SSH_HOST) "cd $(WORK_DIR) && docker compose stop orion_backend_prod"
	@echo "🔍 Проверяем ответ сервера (ждем 502/503 и нашу заглушку)..."
	@curl -s -I http://api.orioncode.ru | grep -E "502|503" || (echo "❌ ОШИБКА: Заглушка не отдается!"; ssh $(SSH_USER)@$(SSH_HOST) "cd $(WORK_DIR) && docker-compose start orion_backend_prod"; exit 1)
	@echo "✅ Заглушка работает! Восстанавливаем бэкенд..."
	@ssh $(SSH_USER)@$(SSH_HOST) "cd $(WORK_DIR) && docker-compose start orion_backend_prod"
	@echo "🚀 Бэкенд снова в строю."
